package lesson05.exceptionHabr;
//
...
//    ничего не
//
//    возвращает(void),но у вам таки есть что сказать —
//    можете бросить
//    исключение.
//
//            Давайте рассмотрим
...
//    площадь прямоугольника
//    с двумя
//    отрицательными сторонами -10и -20=200.
//
//    Мы не
//    можем ничего
//    не вернуть
...
//
//    Можно вернуть
//    специальное значение, показывающее, что
//    что-
//    то не
//
//    так(error code),но кто гарантирует,что его прочитают,а не просто воспользуются им?
//
//    public static int area(int width, int height) {
//        if (width < 0 || height < 0) {
...
//    если обнаружили
//    возможное некорректное
//    поведение,то
//1.
//    Вычисления остановить, сгенерировать
//    сообщение-поломку,
//    которое трудно
//    игнорировать,
...
//    возможность все
//
//    починить(загрузить белье назад и повторно нажать кнопку старт)
//
//    public static int area(int width, int height) {
//        if (width < 0 || height < 0) {
...
//
//4.
//    Нелокальная передача
//
//    управления(nonlocal control transfer)
//
//    Механизм исключительных
//
//    ситуация(исключений) —
//    это механизм
//    НЕЛОКАЛЬНОЙ ПЕРЕДАЧИ
//    УПРАВЛЕНИЯ.
//            Что под
//    этим имеется
//    в виду?
...
//    в ходе
//
//    своего выполнения(точнее исполнения инструкций в рамках отдельного потока),оперирует стеком («стопкой»)
//    фреймов.Передача управления
//    осуществляется либо
//    в рамках
//    одного фрейма
//
...
//    возвращаемся к
//
//    предыдущему фрейму(через return или просто кончились инструкции в методе)
//
//
//return —
//    выходим из
//
//    ОДНОГО фрейма(из фрейма #4(метод h()))
//
//    public class App {
//        public static void main(String[] args) {
...
//    остановить летящее
//
//    исключение(причина, по которой мы автоматически покидаем фреймы).
//    Останавливаем через 3фрейма,
//    пролетаем фрейм #4(
//
//    метод h())+
//    пролетаем фрейм #3(
...
//
//
//    Останавливаем через 1
//
//    фрейм(фактически аналог return, просто покинули фрейм «другим образом»)
//
//    public class App {
//        public static void main(String[] args) {
//            System.err.println("#1.in");
...
//    Они образуют
//    иерархию с
//    корнем java.lang.Throwable(java.lang.Object —
//    предок java.lang.Throwable,
//    но Object —
//    уже не
//    исключение)
//            2.
...
//    иметь поля
//
//    и методы(в этой статье это не будем использовать)
//
//    По первому
//    пункту:catch —
//    полиморфная конструкция, т.е . catch
//    по типу
//    Parent перехватывает
//    летящие экземпляры
//    любого типа, который
//    является Parent-
//
//    ом(т.е.экземпляры непосредственно Parent-а или любого потомка Parent-а)
//
//    public class App {
//        public static void main(String[] args) {
//            try {
...
//    по одному «брату»
//    не может
//    поймать другого «брата» (
//    Error и
//    Exception не
//    находятся в
//    отношении предок-потомок,
//    они из
//    параллельных веток
//    наследования от
//    Throwable)
...
//    хотя у
//    нас летит
//    Error а «ниже»расположен catch
//    по Error.
//    Но важный
//    момент в
//    том,что catch
//    имеет отношение
//    исключительно к try-секции,
//    но не
//    к другим catch-
//    секциям.
//
...
//    ниже —
//    можно строить
//    вложенные конструкции, но
//    вот пример, «исправляющий»
//    эту ситуацию
//
//    public class App {
//        public static void main(String[] args) {
...
//    после брата —
//
//    можно(RuntimeException после Error)
//
//    public class App {
//        public static void main(String[] args) {
//            try {
...
//    осуществляется в
//
//    runtime(а не в compile-time),значит учитывается не тип ССЫЛКИ (Throwable),
//    а тип
//
//    ССЫЛАЕМОГО(Exception)
//
//    public class App {
//        public static void main(String[] args) {
//            try {
...
//
//    Однако finally-
//    секция может «перебить» throw/return
//    при помощи
//    другого throw/return
//
//    public class App {
//        public static void main(String[] args) {
...
//    этих целей
//    в Java 7
//    появилась конструкция try-with-resources,
//    ее мы
//    изучим позже.
//
//    Вообще говоря, в finally-
//    секция нельзя
//    стандартно узнать
//    было ли
//    исключение.
...
//— return из finally-
//
//    секции(можем затереть исключение из try-блока)
//—
//    действия в finally-секции,
//    которые могут
//
//    бросить исключение(можем затереть исключение из try-блока)
//
//
//8.try+catch+finally
//
...
//    после оператора —
//    вылетаем с
//    неперехваченным исключением
//
//
//9.Вложенные try+catch+finally
//
//    Операторы обычно
//    допускают неограниченное
//    вложение .
//    Пример с if
//
//    public class App {
...
//    обе catch-
//
//    секции(нет исключения),заходим в обе finally-
//    секции и
//    выполняем обе
//    строки ПОСЛЕ finally.
//
//    Вложенный try-catch-finally
//    с исключением, которое
//    ПЕРЕХВАТИТ ВНУТРЕННИЙ catch
//
//    public class App {
//        public static void main(String[] args) {
//            try {
...
//    в ПЕРВУЮ catch-
//
//    секцию(печатаем «3»),но НЕ заходим во ВТОРУЮ catch-
//
//    секцию(НЕ печатаем «6», так как исключение УЖЕ перехвачено первым catch),заходим в обе finally-
//
//    секции(печатаем «4» и «7»),в обоих случаях выполняем код после finally(печатаем «5»и «8»,
//    так как
//    исключение остановлено
//    еще первым catch).
//
//    Вложенный try-catch-finally
//    с исключением, которое
//    ПЕРЕХВАТИТ ВНЕШНИЙ catch
//
//    public class App {
//        public static void main(String[] args) {
//            try {
...
//    ПЕРВУЮ catch-
//
//    секцию(не печатаем «3»),но заходим в ВТОРУЮ catch-
//
//    секцию(печатаем «6»),заходим в обе finally-
//
//    секции(печатаем «4» и «7»),в ПЕРВОМ случае НЕ выполняем код ПОСЛЕ finally(
//    не печатаем «5»,
//    так как
//    исключение НЕ
//    остановлено),
